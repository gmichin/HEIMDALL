...
export class TelaNovasMateriasComponent implements OnInit {
  ...
  constructor(
    ...
    this.resgiterForm = this.fb.group(
      {
        name: [this.classToEdit.valid ? this.classToEdit.class.name: '', [Validators.required]],
        description: [this.classToEdit.valid ? this.classToEdit.class.description : '', [Validators.required]],
        teachers_id: [this.classToEdit.valid ? this.classToEdit.class.teachers_id : [], [Validators.required]],
        course_id: [this.classToEdit.valid ? this.classToEdit.class.course_id : '', [Validators.required]],
      }
    );
  }
  ngOnInit(): void {
    this.internsService.getAllTeachers().subscribe({
      next: res => {
        ...
        if(res.length == 0) {
          this.snackBar.open('Ocorreu um erro durante a busca dos professores, por favor, tente novamente mais tarde.', '', {
            duration: 4000,
          });
        }
      },
